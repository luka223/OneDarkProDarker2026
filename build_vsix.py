...
def manifest_json_data(theme: Any) -> str:
    return f"""{{ "id": "{theme.id}", "version": "{theme.version}", "type": "Vsix", "vsixId": "{theme.id}", "extensionDir": "{theme.extension_dir}", "dependencies": {{ "Microsoft.VisualStudio.Component.CoreEditor": "{theme.target_version}" }}, "files": [{{ "fileName": "/extension.vsixmanifest", "sha256": null }}, {{ "fileName": "/extension.pkgdef", "sha256": null }}, {{ "fileName": "/LICENSE.txt", "sha256": null }} {f', {{ "fileName": "/{theme.icon}", "sha256": null }}' if theme.icon else ""}] }}"""
